...
contract StrataForgeAdmin is Ownable, ReentrancyGuard, Pausable {
    ...
    function setFactoryContract(address _factoryContract) external onlyAdmin {
    ...
    function setAirdropContract(address _airdropContract) external onlyAdmin {
        if (_airdropContract == address(0)) revert InvalidAddress();
        airdropContract = _airdropContract;
        emit AirdropContractUpdated(_airdropContract);
    }

    function setFeatureFee(uint256 _feeUSD) external onlyAdmin {
        if (_feeUSD == 0) revert InvalidAmount();
        featureFeeUSD = _feeUSD;
        emit FeatureFeeUpdated(_feeUSD);
    }

    function setAirdropFees(AirdropFeeTier[] calldata newTiers) external onlyAdmin {
    ...
    function proposeWithdrawal(uint96 amount) external onlyAdmin returns (uint32 proposalId) {
    ...
    function approveWithdrawal(uint32 proposalId) external onlyAdmin {
        WithdrawalProposal storage proposal = withdrawalProposals[proposalId];
